= TBD-Verse Protocol Specification
:cddl: ./cddl/
Layer One Labs, LLC
// Metadata
:toc:
...
.TODO:
- [[attribute-index]] Attribute Index Source.
Probably some github repository somewhere.
...
The TBD-Verse itself will be decentralized because of the requirements set forth in this protocol.
...
=== Scope
The TBD-Verse itself is the _result_ of multiple networks implementing this protocol.
Networks MAY be decentralized themselves (e.g. by using a blockchain), or not.
The TBD-Verse itself will be decentralized as a result of social constructs being established between networks and users.

This allows for an heterogeneous global network of

// == Concepts
// === Glossary
// - [[glossary-network]]`Network`.
//     A `network` is a collection of 1 or more servers that act together and collude in some way, receiving messages and executing on them, and returning responses.
//     A `network` has a message queue, and can (or not) be decentralized itself, or implement replication, as it sees fit.
//     A `network` may or may not have a single identity.
//     Each server running in the `network` should have its own private key and thus identity.
// - [[glossary-server]]`Server`.
//     A `server` is a single endpoint that is reachable and can receive messages and return responses.
//     A `server` can be its own network, or be part of a larger network.
// - [[glossary-messages]]`Message`.
//     A `message` contains all information related to a transaction, its source and destination, arguments and expected metadata.
//     It is the main method of communication between networks, and between users and the networks.

== Technical Specification
The TBD-Verse is a global actor-model based network where actors live as federated <<glossary-network,networks>>, and receive and send messages within or across those {network}s.
...
A TBD-Verse request is a Tagged COSE Sign message (https://datatracker.ietf.org/doc/html/rfc8152[RFC 8152]) which contains as its payload a CBOR encoded request defined in this CDDL:
...
=== Attributes
A core tenet of the TBD-Verse is that networks can vary in size, decentralization, governance, capabilities, etc.
