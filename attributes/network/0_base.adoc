...
- <<_status, `status`>>.
    Get the status of the network, its given name (non-unique), identity (unique), public key, and any other information the server wants to make available (free form dictionary).
- <<_echo, `echo`>>.
    Returns the input bytes per bytes, in a signed envelope.
