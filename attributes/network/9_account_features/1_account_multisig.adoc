...
if "canMultisigParticipant" then
    -->[yes] submitted
else
    --> error
    --> (*)
...
=== Roles
This feature adds one new role; `canMultisigParticipant`.

- `"canMultisigParticipate"` is allowed to submit, approve transactions, and revoke their own approvals.
    Participant can withdraw their own submitted transactions as well.
...
=== Defaults
By default, on account creation, the number of approval needed for any transaction will be the total number of identities with the `"canMultisigParticipant"` role on the account (including `"owner"`).
