...
* `account.delete`.
    Deletes or disables an account.
    The identity may not be reusable.
    The server MAY still return the info using `account.info`, with the disabled field filled.
    For all other endpoints, there is no functional differences between a deleted or disabled account.
* `account.addFeatures`
...
==== Owner Role
An account MUST have at least one owners, with their role set including `"owner"`.
Removing the `"owner"` role from the last `"owner"` identity on an account MUST result in an error.

By default, only an owner can set roles of identities on an account, or delete it.
...
[sources,cddl]
----
include::{cddl}/9_account.cddl[tag=delete]
----
